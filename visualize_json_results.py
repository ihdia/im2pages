...
from immi_dataset import register_dataset
# from immi_dataset import register_dataset
...
class CustomVisualizer(Visualizer):
    ...
    def draw_polygon(self, segment, color, edge_color=None, alpha=0.5,linestyle='dashed'):
        """
        Args:
    ...
    def overlay_instances(
        self, *, boxes=None, labels=None, masks=None, keypoints=None, assigned_colors=None, alpha=0.5,linestyle
    ):
        """
        Args:
        ...
        for i in range(num_instances):
            ...
            if masks is not None:
                for segment in masks[i].polygons:
                    self.draw_polygon(segment.reshape(-1, 2), color, alpha=alpha,linestyle=linestyle)
...
if __name__ == "__main__":
    ...
    for dic in tqdm.tqdm(dicts):
        ...
        vis = CustomVisualizer(vis_pred, metadata)
        # vis_gt= vis.draw_instance_predictions(dic['annotations'][0]).get_image()
        ...
        cv2.imwrite(os.path.join(args.output, filename), vis_gt[:, :, ::-1])
